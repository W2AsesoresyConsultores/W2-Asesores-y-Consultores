import React from "react";

function CtaAreas() {
  return (
    <div id="areas" class="bg-white dark:bg-gray-800 font-dmsans">
      <div class="text-center w-full mx-auto py-12 px-4 sm:px-6 lg:pt-16 lg:px-8">
        <h2 class="text-3xl font-extrabold text-black dark:text-white sm:text-5xl">
        <img src="https://elcuvegbwtlngranjtym.supabase.co/storage/v1/object/public/AssetsW2/Inicio/flecha.png" alt="" className="inline-block w-16 h-12 mr-2"/>Descubre nuestras áreas,
          <span class=" text-primarycolor"> sé parte de nuestro equipo</span>
        </h2>
        <p class="text-xl mt-4  mx-auto text-gray-600">
          Emocionantes proyectos, mentoría de expertos y un ambiente inclusivo
          que fomenta el aprendizaje y el crecimiento profesional
        </p>
      </div>
    </div>
  );
}

export default CtaAreas;
